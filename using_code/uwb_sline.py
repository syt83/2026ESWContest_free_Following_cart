...
class UwbImuFollowerMaster(Node):
    def __init__(self):
        ...
        # 🧠 상태 머신 및 방향 제어 변수
        # ==========================================
        # 💡 [수정됨] 파이썬 시작 시 무조건 360도 스캔부터 하도록 수정!
        self.mode = 'SCAN_360'         
        self.fail_count = 0            
        self.steer_dir = 1             
    ...
    def init_arduino_serial(self):
        try:
            ...
            time.sleep(2)  
            self.get_logger().info('✅ 아두이노 연결! 🚀최초 360도 스캔 후 추종 시작!')
    ...
    def uwb_callback(self, msg: Point):
        ...
        if self.is_stopped:
            ...
            if dist_cm < self.STOP_DIST:
                ...
                # 다시 쫓아갈 때는 360도 스캔을 건너뛰고 바로 TRACK으로 들어감
                self.mode = 'TRACK'
                self.get_logger().info(f'🛑 목표 도달 정지 (거리: {dist_cm:.0f}cm)')
                return

        # ==========================================
        # 🔍 [모드 1] SCAN_360 (프로그램 최초 실행 시 1회 발동)
        # ==========================================
        ...
        # ==========================================
        # 🐍 [모드 4] TRACK : 묵직한 S자 + 절대 U턴 방어 로직
        # ==========================================
        elif self.mode == 'TRACK':
            
            # U턴 전용 락: U턴 중이면 딴짓 절대 안 함!
            if self.is_uturning:
                yaw_error = self.normalize_angle(self.target_yaw - self.current_yaw)
                
                # U턴을 다 돌았을 때 (오차 15도 이내)
