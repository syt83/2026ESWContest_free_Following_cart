...
**센서:** LiDAR (YDLIDAR X4 Pro) / UWB 모듈 (Arduino Stella, 로봇 탑재) / IMU / NFC Reader (RC522)

**구동부:** DC Motor ×2 / Motor Driver (MD20A)
...
**⑥ UWB range-only 기반 사용자 위치 추적**
- 단일 UWB 모듈(Stella)만으로 사용자 위치 추정
- 초기 위치 보정 → 지속 추적 → 로컬 목표 지점 갱신
...
설계 항목: 카트 프레임 / LiDAR 거치대 / NFC 모듈 거치대 / 보드용·모터용 배터리 홀더(분리) / 전자부품 보호 케이스 / 케이블 정리 구조 / UWB 리모컨 거치대
